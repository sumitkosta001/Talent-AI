...
export default function LoginForm() {
  ...
  const { rememberMe, savedEmail, toggleRemember } = useRememberMe();
  
  const [email, setEmail] = useState(savedEmail || 'alex@example.com');
  ...
  const handleLogin = async (role: 'candidate' | 'recruiter') => {
    ...
    try {
      toggleRemember(rememberMe, email);
    ...
    <div className="w-full space-y-4 text-left">
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className="flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
        >
      ...
      <div className="relative flex items-center my-3">
        ...
        <span className="px-3 text-[11px] text-slate-400 font-semibold uppercase tracking-wider">or email credentials</span>
        <div className="flex-1 border-t border-slate-200 dark:border-slate-800" />
      </div>
      ...
      <RememberMe checked={rememberMe} onChange={(val) => toggleRemember(val, email)} />
      ...
      <div className="space-y-2 pt-2">
        <button
          ...
          onClick={() => handleLogin('candidate')}
          disabled={loading}
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl text-xs sm:text-sm shadow-sm transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
        >
        ...
        <button
          ...
          disabled={loading}
          className="w-full border border-blue-600 dark:border-blue-500 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/20 font-bold py-3 rounded-xl text-xs sm:text-sm transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
        >
